...
MediaPlayerProxy::MediaPlayerProxy() :
	...
	QObject::connect(backend.get(), &QMediaPlayer::mediaChanged,
			[=](const QMediaContent &media) {
				qCDebug(CLASS_LC)
				<< "MediaPlayerProxy media_changed()";
				/*
	...
	QObject::connect(backend.get(), &QMediaPlayer::metaDataAvailableChanged,
			[=](bool available) {
				qCDebug(CLASS_LC)
				<< " metaDataAvailableChanged " << available;
				if (available) {
					QString title =
							backend->metaData(QMediaMetaData::Title).toString();
					QString publisher = backend->metaData(
							QMediaMetaData::Publisher).toString();
...
void MediaPlayerProxy::do_set_media(
	...
	qCDebug(CLASS_LC)
	<< Q_FUNC_INFO << media->get_display_name();
	current_item = media;
	...
	if (previous_position != 0) {
		qCDebug(CLASS_LC)
		<< "restarting from position" << previous_position;
...
void MediaPlayerProxy::do_increment_volume(int increment) {
	qCDebug(CLASS_LC)
	<< Q_FUNC_INFO << increment;
	qCDebug(CLASS_LC)
	<< " current volume" << linear_volume;
