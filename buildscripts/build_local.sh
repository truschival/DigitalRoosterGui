...
export BUILD_DIR=/build_in_container
...
log_step "Check & pull podman image"
podman --version
podman pull $BUILD_IMAGE

log_step "Start container $CONTAINER_NAME"
podman run -itd -u $UID:$GID --privileged --name $CONTAINER_NAME  \
...
podman exec $CONTAINER_NAME cmake \
...
podman exec $CONTAINER_NAME cmake --build $BUILD_DIR --parallel

log_step "$CONTAINER_NAME: Get openapi client"
podman exec -w $SRC_DIR $CONTAINER_NAME buildscripts/get_openapi_client.py

log_step "$CONTAINER_NAME: Run Tests"
podman exec $CONTAINER_NAME cmake --build $BUILD_DIR --target test
...
podman exec -w $BUILD_DIR -eDESTDIR=$INSTALL_DIR $CONTAINER_NAME \
       cmake --build . --target install

log_step "$CONTAINER_NAME: Gather coverage stats"
podman exec -w $BUILD_DIR  $CONTAINER_NAME \
