...
bool operator==(QString qs , char* c ){return qs.toStdString().c_str() == c; }
...
TEST(PodcastSourceReader,parseInfo_bad) {
	...
	PodcastSourceConfiguration ps(
			"https://alternativlos.org/alternativlos.rss");
	ps.set_rss_feed("./alternativlos_bad.rss");
...
TEST(PodcastSourceReader,getEpisodes) {
	...
	EXPECT_STREQ(episodes[1]->get_display_name().toStdString().substr(0,6).c_str(),"ALT040");
	EXPECT_STREQ(episodes[1]->get_url().toString().toStdString().c_str(), "http://alternativlos.cdn.as250.net/alternativlos-40.mp3");
}
